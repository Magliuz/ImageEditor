...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['top'], $_FILES['left'], $_FILES['right'])) {
        ...
        $topPath   = $uploadDir . basename($_FILES['top']['name']);
        $leftPath  = $uploadDir . basename($_FILES['left']['name']);
        $rightPath = $uploadDir . basename($_FILES['right']['name']);

        move_uploaded_file($_FILES['top']['tmp_name'], $topPath);
        move_uploaded_file($_FILES['left']['tmp_name'], $leftPath);
        move_uploaded_file($_FILES['right']['tmp_name'], $rightPath);

        try {
            $faces = ['top' => $topPath, 'left' => $leftPath, 'right' => $rightPath];
            $images = [];
